...
!(function (a, b) {
	...
	function c(a, c) {
		function d(a, b, c, d) {
			var e = ''
			;(e = b.length ? b + '{' + c + '}' : c),
				'insertRule' in a ? a.insertRule(e, d) : 'addRule' in a && a.addRule(b, c, d)
		}
		var e = null,
			f = null,
			g = null
		if (('string' == typeof a && (e = b.getElementById(a.split('#')[1])), !e))
			return void console.error('Grained: cannot find the element with id ' + a)
		;(f = e.id),
			'absolute' !== e.style.position && (e.style.position = 'relative'),
		...
		s && s.parentElement.removeChild(s),
			(r = b.createElement('style')),
			(r.type = 'text/css'),
			(r.id = 'grained-animation-' + f),
			b.body.appendChild(r)
		var t = 'background-image: url(' + k + ');'
		if (
			((t += 'position: absolute;content: "";height: 300%;width: 300%;left: -100%;top: -100%;'),
			(n = h.length),
			i.animate)
		)
			for (; n--; )
