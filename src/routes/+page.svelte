<script lang="ts">
	...
	let isBackgroundsOpen = $derived($OpenShells.find((shell) => shell.id === 'backgrounds'))
	...
	const create3DBackground = (node: HTMLCanvasElement) => {
		...
		setTimeout(() => {
			spline.load('https://prod.spline.design/0JtwT9xb53fikO5h/scene.splinecode').then(() => {
		...
		<div class="h-[calc(100vh-6rem)] overflow-hidden">
			<div class="page-wrapper">
				{#if $SelectedBackground?.name === 'bg-scene'}
					<div
						class={twm(
							'sand-transition sticky bottom-0 top-0 h-screen',
							isSplineLoaded && isSplineThemeChangeComplete ? 'opacity-100' : 'opacity-0'
						)}
					>
				...
				<div class="absolute left-12 top-12 flex select-none flex-col items-start text-[1.375rem]">
					...
					<div class="overflow-hidden">
						<div
							use:sandTextLottie
							class="fade-up h-[16rem] max-w-[32rem] scale-[1.026] dark:invert"
						></div>
					</div>
					...
					<div class="mt-12 flex flex-col items-start gap-2">
						<button
							...
							class="fade-up sand-transition leading-none text-light-80 hover:text-black dark:text-light-25 dark:hover:text-light-100"
						>
							Privacy Policy
						</button>
						<button
						...
						<a
							href="/"
							class="fade-up sand-transition mt-5 leading-none text-light-80 hover:text-black dark:text-light-25 dark:hover:text-light-100"
						>
				...
				<button
					...
					class={twm(
						'ade-up group fixed left-1/2 top-[8rem] flex translate-x-[7rem] items-center gap-3',
						$SelectedBackground?.name === 'bg-scene' && 'hidden'
					)}
				>
					<div class="sand-transition h-[3.44rem] w-[3.44rem] shrink-0 group-hover:text-sand-green">
						<svg
							width="100%"
							height="100%"
							viewBox="0 0 55 55"
					...
					<div
						class="sand-transition relative text-base text-sand-green opacity-0 group-hover:opacity-100"
					>
						Virtual Assistant
					</div>
				</button>
				...
				<div class="fade-up pointer-events-none h-[calc(100vh-3rem)]">
					{#if $SelectedBackground?.name === 'bg-legacy'}
						<div
							class={twm(
								'sand-transition flex h-full flex-col justify-between overflow-hidden dark:invert',
								loadedLegacyLotties.length === 3 ? 'opacity-10 dark:opacity-20' : 'opacity-0'
							)}
						>
...
{#if isMobile}
	...
	<MobileAppShell>
		...
		<div class="swiper relative pb-3">
			<div class="swiper-wrapper flex h-[calc(100dvh-9rem)] pb-3 pt-2">
				<SlideHome />
				<SlideMenu />
				<SlideAbout />
