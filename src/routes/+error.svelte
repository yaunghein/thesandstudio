<script lang="ts">
	...
	const animateLogo = (e: MouseEvent) => {
		...
		const outerShapes = document.querySelectorAll('.logo-shape') as NodeListOf<HTMLElement>
		...
		const mouthShapes = document.querySelectorAll('.logo-mouth') as NodeListOf<HTMLElement>
		...
		const eyesShapes = document.querySelectorAll('.logo-eyes') as NodeListOf<HTMLElement>
	...
	onMount(() => {
		...
		!document.documentElement.classList.contains('dark') && logoShapeLottie.goToAndStop(100, true)
...
<AppShell>
	<div class="grid h-full place-items-center bg-white dark:bg-black">
		{#if page.status === 404}
			<div class="flex flex-col gap-2 text-center sm:gap-7">
				<div class="relative">
					<div class="flex items-center gap-20 sm:gap-[15rem]">
						<div
							class="font-sand-black text-9xl leading-none text-light-40 drop-shadow-404-dark dark:text-white dark:drop-shadow-404-light sm:text-[32rem]"
						>
							4
						</div>
						<div
					...
					<div
						class="absolute -left-[35rem] top-[8rem] aspect-square w-[60rem] bg-404-gradient-light dark:bg-404-gradient-dark"
					></div>
					<div
						class="absolute -right-[35rem] top-[8rem] aspect-square w-[60rem] bg-404-gradient-light dark:bg-404-gradient-dark"
					></div>
					<div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
						<button
							onclick={stopPropagation(switchMode)}
							class="main-logo relative aspect-square w-[8rem] -translate-y-2 drop-shadow-404-dark invert dark:drop-shadow-404-light dark:invert-0 sm:w-[25rem]"
						>
							<div class="logo-shape"></div>

							<div
								class="absolute left-1/2 top-1/2 -mt-[0.5rem] aspect-auto w-10 -translate-x-1/2 -translate-y-1/2 opacity-30 sm:-mt-[1.5rem] sm:w-28"
							>
								<svg
									width="100%"
									height="100%"
							...
							<div
								class="absolute left-1/2 top-1/2 mt-[1rem] aspect-auto w-10 -translate-x-1/2 -translate-y-1/2 opacity-30 sm:mt-[3rem] sm:w-36"
							>
								<svg
									width="100%"
									height="100%"
