import type { WorkGroup, Work } from '$lib/types'
...
function getCurrentAndNextProject(works: Work[], slug: string) {
	const index = works.findIndex((project) => project.slug === slug)
	if (index === -1) return null

	const currentProject = works[index]
	const nextProject = works[(index + 1) % works.length]
...
export const load = async ({ params }) => {
	const COMBINED_WORKS = [...WORKS_GROUPS[0].data, ...WORKS_GROUPS[1].data]
	const data = getCurrentAndNextProject(COMBINED_WORKS, params.slug)
	if (!data) {
