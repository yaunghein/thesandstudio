<script lang="ts">
	...
	const getFiles = (file: any) => file.data?.filter((i: any) => i.type === 'file')
	const getFolders = (file: any) => file.data?.filter((i: any) => i.type === 'folder')
</script>
		...
		{#if files[0]}
			{#each files as file}
				<button
					title={file.label}
					class={twm(
						'relative -mt-[2px] flex h-10 w-full items-center justify-between gap-1 rounded-[1.2rem] px-5 text-left text-2xl leading-none',
						file.isOpen && 'bg-black text-white dark:bg-white dark:text-black'
					)}
					id={file.label}
				...
				>
					...
					{#if file.type === 'folder'}
						<span class="absolute right-5 shrink-0">
							{#if !file.isOpen}
								<svg
									width="10"
									height="9"
							...
							{:else}
								<svg
									...
									xmlns="http://www.w3.org/2000/svg"
								>
									<path d="M7.38965 1.89526H0.819824V0.25H7.38965V1.89526Z" fill="currentColor" />
								</svg>
							{/if}
						</span>
					{/if}
				...
				{#if file.isOpen}
					...
					<div
						class="grid {file.label === 'Sawbwa Morning Blend'
							? 'grid-cols-2'
							: 'grid-cols-1'} gap-[2px] p-1 pt-[2px] align-top"
					>
						{#each medias as media}
							{#if media.mediaType === 'img'}
								<img alt={media.label} src={media.mediaSrc} class="rounded-lg" />
							{:else if media.mediaType === 'video'}
								<div class="w-full bg-black">
									<video style="width:100%;height:100%;" autoplay loop src={media.mediaSrc}>
										<source type="video/mp4" src={media.mediaSrc} />
										<track kind="captions" />
									</video>
								</div>
							{:else if media.mediaType === 'pdf'}
								<a class="p-4 text-center leading-[0.75]" href={media.mediaSrc} target="_blank"
									>{media.label}</a
								>
							{/if}
						{/each}
