...
<div
	class="-ml-[0.125rem] -mt-[0.1rem] h-full w-[29.8rem] shrink-0 border-l-2 border-white p-5 dark:border-light-12"
>
	{#if data.label.toLowerCase() === 'we are sand'}
		<div
			class="h-full w-full overflow-auto rounded-xl border-2 border-white p-5 dark:border-light-12"
		>
			<p class="text-xl">
				At SAND, we are more than just a design studio; we are a collective of innovative spirits
				venturing where most have traveled less. Our team embodies a fusion of talents in
				Architecture, Design, Motion, and Code. We are the home for the curious, the thinkers, the
				makers, and the doers who are passionate about transforming bold ideas into tangible
				realities. Our ethos is rooted in pushing boundaries and delving into the intersections of
				diverse fields. We don't just create; we collaborate, innovate, and inspire. SAND is where
				like-minded individuals unite to challenge the conventional and explore uncharted
				territories in design and technology. We are on a journey to a destination unknown, driven
				by the belief that it takes a multitude of fields and diversity to achieve something truly
				groundbreaking. Our projects are collaborative explorations, valuing each unique voice and
				idea, igniting the potential for extraordinary innovation. Join us at SAND, where your
				passion for creativity and diversity finds its purpose. Here, we're not just designing for
				today; we're pioneering the future through interdisciplinary collaboration and collective
				ingenuity. We are going where most have traveled less, because it takes a rich tapestry of
	...
	{:else if data.label.toLowerCase() === 'our services'}
		...
		>
			<div class="grid gap-6">
				<div>
					<p class="text-xl leading-tight">Branding and Graphic Design</p>
					<p class="text-xl leading-tight">Motion Graphics and Animation</p>
				</div>
				<div>
					<p class="text-xl leading-tight">Architectural Design</p>
					<p class="text-xl leading-tight">Interior Design</p>
					<p class="text-xl leading-tight">3D Modeling and Visualization</p>
				</div>
				<div>
					<p class="text-xl leading-tight">Web Design and Development</p>
					<p class="text-xl leading-tight">Web Maintenance and Support</p>
					<p class="text-xl leading-tight">E-commerce Solutions</p>
				</div>
				<div>
					<p class="text-xl leading-tight">Consultation and Strategy</p>
				</div>
				<div>
					<p class="text-xl leading-tight">Others</p>
					<p class="text-xl leading-tight">
						(Every idea matters to us – no concept too small, no vision too grand. If you’re a
	...
	{:else if data.label.toLowerCase() === 'inquiry'}
		...
		>
			<div class="grid gap-6">
				<div>
					<p class="text-xl leading-tight">General:</p>
					<a href="mailto:hi@thesandstudio.com" class="text-xl leading-tight">
	...
	{:else if data.label.toLowerCase() === 'social media'}
		...
		>
			<div class="grid gap-6">
				<div class="grid">
					<a href="https://www.google.com/" target="_blank" class="text-xl leading-tight">
						Instagram
					</a>
					<a href="https://www.google.com/" target="_blank" class="text-xl leading-tight">
						Facebook
	...
	{:else if apps.includes(data.label)}
		<div
			class="flex h-full flex-col overflow-hidden rounded-2xl border-2 border-white dark:border-light-12"
		>
			<div
				class="relative grid aspect-square w-full scale-[1.008] items-center overflow-hidden rounded-2xl border-2 border-t-0 border-white dark:border-light-12"
			>
				<div class="{paddings[data.label]} invert dark:invert-0" id="lottie-container">
			...
			<div
				class="-m-[0.1875rem] grow overflow-hidden rounded-b-2xl border-2 border-t-0 border-white dark:border-light-12"
			>
				<p class="p-6 text-xl leading-tight">
					{data.description || ''}
				</p>
			</div>
			<button class="h-20 w-full shrink-0 font-sand-medium text-xl" onclick={data.handleClick}>
				{data.label === 'Sand Scan' ? 'Launch' : 'Visit'}
			</button>
		</div>
	{:else}
		<div
			class="flex h-full flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-white dark:border-light-12"
		>
			<button onclick={() => handleFileDoubleClick(data)}>
				{#if data.mediaType === 'img'}
					<img src={data.mediaSrc} alt={data.label} class="w-full rounded-2xl object-contain" />
				{:else if data.mediaType === 'video'}
					<div class="group relative h-full w-full">
						{#if data.thumbnail}
							...
							/>
							<div
								class="sand-transition absolute left-1/2 top-1/2 flex aspect-square w-32 -translate-x-1/2 -translate-y-1/2 items-center text-black opacity-50 group-hover:opacity-100"
							>
								<svg
									width="100%"
									height="100%"
						...
						{:else}
							<div
								class="sand-transition absolute left-1/2 top-1/2 flex aspect-square w-32 -translate-x-1/2 -translate-y-1/2 items-center text-black opacity-50 group-hover:opacity-100 dark:text-light-100"
							>
								<svg
									width="100%"
									height="100%"
