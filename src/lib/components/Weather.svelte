	...
	<div class="relative">
		<div class="relative flex gap-10">
			<div class="flex flex-col items-center justify-start gap-2">
				<div class="relative -mt-5 aspect-square w-28 overflow-hidden">
					{#if icon}
						{#key icon}
							<img
				...
				<div class="flex gap-2 text-4xl leading-none {MARGIN_TOPS[icon?.size] || ''}">
					<div>
					...
					<div class="flex items-center gap-2">
						...
						<div class="-mt-1 h-8 w-[0.25rem] shrink-0 bg-light-80"></div>
						<button
							onclick={() => (unit = 'F')}
							class={twm(
								unit === 'F'
			...
			<div class="flex flex-col gap-2">
				<div class="text-[1.375rem]">
					...
					<div class="mt-2 leading-none">{weather.current.condition.text}</div>
				</div>

				<div class="flex flex-col gap-2 text-[1.375rem] text-black dark:text-white">
					...
					>
						<div
							class={twm(
								...
								'sand-transition aspect-square w-[6px] shrink-0 rounded-full bg-light-10 dark:bg-light-100'
							)}
						></div>
						<div
							class={twm(
								'sand-transition text-[1.375rem]',
