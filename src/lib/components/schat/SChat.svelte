	...
	run(() => {
		if (messages.length > 2) {
			...
			setTimeout(() => {
				...
				const messageElHeight1 = messageEls[messageEls.length - 1].getBoundingClientRect().height // last message
				const messageElHeight2 = messageEls[messageEls.length - 2].getBoundingClientRect().height // second last message
...
<div
	...
	class="fixed left-1/2 top-1/2 z-10 h-shell-desktop w-[45.5rem] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-3xl border-2 border-white text-light-10 dark:border-light-12 dark:text-light-100"
	style="z-index: {shell?.zIndex}"
>
	<div class="transparent-layer"></div>

	<div class="absolute inset-0 bottom-auto -ml-[0.9rem] flex h-[5.1rem] gap-10 opacity-sand">
		{#each [...Array(100).keys()] as _}
			<div class="ml-[0.26rem] h-full w-line shrink-0 bg-white dark:bg-light-12"></div>
		{/each}
	</div>
	<div class="absolute inset-0 top-auto -ml-[0.9rem] flex h-[8.3rem] gap-10 opacity-sand">
		{#each [...Array(100).keys()] as _}
			<div class="ml-[0.26rem] h-full w-line shrink-0 bg-white dark:bg-light-12"></div>
		{/each}
	</div>
	...
	<div class="relative flex h-full flex-col overflow-hidden">
		<div class="z-2 absolute left-6 top-5">
			<ButtonClose
				on:close={() => {
					removeShell('schat')
				}}
		...
		<div class="absolute right-4 top-[1.05rem] flex gap-3">
		...
		<div
			class="non-draggable mx-3 mt-20 grow cursor-default select-text overflow-hidden rounded-[1.25rem] border-2 border-white bg-light-90 bg-opacity-75 text-light-10 dark:border-light-12 dark:bg-light-4 dark:bg-opacity-75 dark:text-light-100"
		>
			<div class="flex h-full flex-col p-6 pb-0 pt-0">
				<div
					id="chat-container"
					class="hide-scrollbar flex grow flex-col gap-5 overflow-scroll pb-8 pt-6"
				>
					{#if isSchatPolicyOpen}
						<div class="flex flex-col pt-1">
							<div class="mb-1 font-sand-bold text-xl text-black dark:text-light-100">
								Privacy & AI Disclosure
							</div>
							<p class="text-xl">
								Welcome to our virtual assistant service. Your privacy and the security of your data
								are of utmost importance to us. This disclosure outlines how we handle your data and
								the AI technology powering our virtual assistant.
							</p>
						</div>
						<div class="flex flex-col pt-1">
							<div class="mb-1 font-sand-bold text-xl text-black dark:text-light-100">
							...
							<p class="text-xl">
								Your interactions with our virtual assistant are recorded for quality assurance and
								service improvement purposes. The data collected from these interactions may be used
								to enhance our services, improve response accuracy, and train our AI systems. We are
								committed to protecting your privacy and ensuring that your data is handled
								securely.
							</p>
						</div>
						<div class="flex flex-col pt-1">
							...
							<p class="text-xl">
								This virtual assistant utilizes advanced artificial intelligence technology based on
								OpenAI's GPT-4 model. The AI is designed to provide helpful and accurate responses
								based on the data it has been trained on. However, please note that the AI may not
								always provide complete or correct information. For critical or sensitive
								information, we recommend verifying with authoritative sources or seeking
								professional advice.
							</p>
						</div>
						<div class="flex flex-col pt-1">
							...
							<p class="text-xl">
								By using this chatbot, you consent to the recording and use of your interactions as
								described above. Your continued use of the service implies acceptance of these
								terms.
							</p>
						</div>
						<div class="flex flex-col pt-1">
							...
							<p class="text-xl">
								We employ robust security measures to safeguard your data against unauthorized
								access and breaches. For more information on our data handling practices and
								security measures, please refer to our
								<button onclick={openPrivacyPolicy} class="underline"> Privacy Policy </button>.
							</p>
						</div>
						<div class="flex flex-col pt-1">
							<div class="mb-1 font-sand-bold text-xl text-black dark:text-light-100">
							...
							<p class="text-xl">
								While our AI strives to provide accurate and helpful information, it is not
								infallible. The responses generated by the AI are based on the data it has been
								trained on and may not reflect the most current information or specific details of
								your unique situation. For critical information, please consult professional or
						...
						<div class="flex flex-col pt-1">
							...
							<p class="text-xl">
								If you have any questions or concerns about our Privacy & AI Disclosure or how your
						...
						<div class="flex flex-col pt-1">
							<p class="text-xl">
								Thank you for using our virtual assistant service. Your trust and satisfaction are
								important to us.
							</p>
						</div>
					{:else}
						<SChatStartMessage on:policyClick={() => (isSchatPolicyOpen = true)} />
						{#each messages.slice(1) as { role, content }}
							<div class="message-container origin-left" transition:fade={{ duration: 400 }}>
				...
				{#if !isSchatPolicyOpen}
					<form
						...
						class="mb-6 mt-auto flex h-16 w-full shrink-0 items-center gap-2 rounded-full border-2 border-white px-5 dark:border-light-12"
					>
						<input
							bind:this={input}
							disabled={isAsking}
							type="text"
							class="h-full w-full appearance-none bg-transparent text-xl outline-none placeholder:text-xl placeholder:text-white dark:placeholder:text-light-25"
							bind:value={content}
						/>
						<button
							...
							class={twm(
								...
								'sand-transition  relative aspect-square shrink-0',
								isAsking && messages.length > 1 ? 'w-8 text-light-4 dark:text-light-100' : 'w-7'
							)}
						>
		...
		<div class="mt-auto flex shrink-0 items-end justify-between">
			<div class="flex flex-col items-start gap-1 py-6 pl-[2.35rem]">
				<button
					onclick={() => {
						addShell({ id: 'finder', zIndex: 65 })
						openContactTab()
					}}
					class="sand-transition text-xl leading-none text-light-80 hover:text-black dark:text-light-25 dark:hover:text-light-100"
				>
					hi@thesandstudio.com
				</button>
				<button
				...
				<div class="mt-[2px] font-sand-bold text-2xl leading-none text-sand-green">SCHAT</div>
			</div>
			<div class="flex flex-col pb-4 pr-4">
				<div
					class="sand-transition ml-2 max-w-[8rem] text-xs leading-none text-light-80 hover:text-black dark:text-light-25 dark:hover:text-light-100"
				>
					This chatbot is powered by OpenAI’s GPT-4o.
				</div>
				<div class="mt-2 h-[1.88rem] w-[9.72rem] dark:invert">
					<svg
						width="100%"
						height="100%"
						viewBox="0 0 156 30"
