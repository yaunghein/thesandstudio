<script lang="ts">
	...
	let {
		role = 'assistant',
		content = `In the context of our company, "The Sand Studio" is a creative agency crafted by talented individuals from around the globe, based in Bangkok. We offer a wide range of design, development, and consultation services.\n\nIf you\'re inquiring about the material "sand," it commonly refers to granular material composed of finely divided rock and mineral particles, typically found on beaches and deserts.\n\nHow else may I assist you today?`
	}: Props = $props()
...
<div class="flex items-start gap-4">
	<div class="aspect-square w-[2.13rem] shrink-0 text-black dark:text-light-100">
		{#if isAI}
			<svg
				width="100%"
				height="100%"
		...
		{:else}
			...
			>
				<path
					d="M17.1511 29.3022C23.862 29.3022 29.3022 23.862 29.3022 17.1511C29.3022 10.4402 23.862 5 17.1511 5C10.4402 5 5 10.4402 5 17.1511C5 23.862 10.4402 29.3022 17.1511 29.3022Z"
					fill="currentColor"
				/>
			</svg>
		{/if}
	...
	<div class="flex flex-col pt-1">
		<div
			class={twm(
				isAI ? 'text-sand-green dark:text-sand-green' : 'text-black dark:text-light-100',
				'mb-[2px] font-sand-bold text-xl'
			)}
		>
			{isAI ? 'Schat' : 'You'}
		</div>
		<p
			class="grid gap-2 text-xl [&_a]:underline [&_h1]:font-sand-bold [&_h2]:font-sand-bold [&_h3]:font-sand-bold [&_h4]:font-sand-bold [&_h5]:font-sand-bold [&_h6]:font-sand-bold [&_ol]:ml-4 [&_ol]:list-disc [&_strong]:font-sand-bold [&_ul]:ml-4 [&_ul]:list-disc"
		>
