<script lang="ts">
	...
	const animateElements = (elements: NodeListOf<HTMLElement>, properties: object) => {
	...
	const animateLogo = (e: MouseEvent) => {
		if (!isPostLoadingFinished) return

		const rect = document.querySelector('.loading_logo-container')!.getBoundingClientRect()
		const track = {
	...
	onMount(() => {
		...
		logoShapeLottie = lottie.loadAnimation({
			container: document.querySelector('.loading_logo-shape') as HTMLDivElement,
			renderer: 'svg',
			loop: false,
			autoplay: false,
		...
		const progressTl = gsap.timeline({
			...
			onComplete: () => {
				isLoading = false
				logoShapeLottie.removeEventListener('complete', handleLogoShapeLottieLoop)
				logoShapeLottie.goToAndStop(logoShapeLottie.totalFrames - 1, true)
				helloLottie.play()
				const postLoadingTl = gsap.timeline({
					onComplete: () => {
						isPostLoadingFinished = true
					}
				})
				postLoadingTl
					...
					.to('.loading_logo-eyes', { opacity: 1, duration: 1.5, ease: 'power4.out' }, '<')
			}
		})
		progressTl
		...
		logoShapeLottie.addEventListener('DOMLoaded', () => {
			if (show) {
				logoShapeLottie.play()
				progressTl.play()
			} else {
	...
	const justHandleClick = () => {
		if (isLoading) return

		internalShow = false
		gsap.set('.loading_container', { pointerEvents: 'none' })
		gsap.set('.loading_logo-container', { pointerEvents: 'none' })
...
<div class="group">
	<button
		...
		class={twm(
			...
			show ? 'top-[45%] w-[9.75rem] -translate-y-1/2' : 'top-[8rem] w-[13rem]',
			'loading_logo-container sand-transition fixed left-1/2 z-[54] aspect-square -translate-x-1/2 invert-0'
		)}
	>
		<div class="sand-transition group-hover:scale-105">
			<div class="loading_logo-shape"></div>
		</div>

		<div
			class="loading_click-me absolute -right-[4rem] top-[0.5rem] h-[5.88rem] w-[4rem] invert dark:invert-0"
		>
		...
		<div
			class={twm(
				...
				'loading_logo-mouth  absolute inset-0 h-full w-full'
			)}
		>
			<div class="absolute bottom-[30%] left-1/2 w-[40%] -translate-x-1/2 text-black">
				<svg
					width="100%"
					height="100%"
					viewBox="0 0 93 51"
		...
		<div class="absolute inset-0 h-full w-full">
			<div class="absolute bottom-[50%] left-1/2 w-[12%] -translate-x-1/2 text-black">
				<div class={twm(show ? 'opacity-0' : 'opacity-100', 'loading_logo-eyes ')}>
					<svg
						width="100%"
						height="100%"
						viewBox="0 0 26 22"
	...
	{#if !internalShow}
		<div
			class="sand-transition pointer-events-none fixed inset-0 z-[53] h-full w-full bg-light-4 opacity-0 group-hover:opacity-50 dark:group-hover:opacity-30"
		></div>
	{/if}
</div>

{#if show}
	<div class="loading_container fixed inset-0 z-[52] grid h-full w-full place-items-center">
		<div class="loading_bg absolute inset-0 h-full w-full bg-light-95 dark:bg-light-4"></div>

		<div
			class="loading_hello pointer-events-none absolute left-1/2 top-1/2 aspect-square w-[72rem] -translate-x-1/2 -translate-y-1/2 opacity-0 invert dark:invert-0"
		></div>

		<div
			class="loading_bar absolute left-1/2 top-[55%] h-2 w-[22.5rem] -translate-x-1/2 -translate-y-1/2 rounded-full border border-black border-opacity-[0.15] bg-light-95 dark:border-white dark:bg-light-4"
		>
			<div class="absolute -inset-[1px] h-2 w-[22.5rem] overflow-hidden rounded-full">
				<div
