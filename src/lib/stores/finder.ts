		...
		files: [
			{
				...
				description: 'Barrier-grid animation and stereography automation software.',
				mediaType: 'img',
				mediaSrc: ''
			},
			{
...
export const FinderOpenPath = derived([Tabs, FilePreview], ([$tabs, $preview]) => {
...
export const FilePreviewHistory: Writable<(File | undefined)[]> = writable([undefined])
const PreviousSelectedFileLevel: Writable<number> = writable(-1)

export const handleFileClick = (file: File) => {
	const selectedFileLevel = getNestedLevel(get(Tabs).find((tab) => tab.isOpen)!.files, file.id)
	...
	Tabs.update((tabs: Tab[]) => {
		const findAndUpdateIsOpen = (files: File[]) => {
			for (const item of files) {
				...
				const itemLevel = getNestedLevel(get(Tabs).find((tab) => tab.isOpen)!.files, item.id)
